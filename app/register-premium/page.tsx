...
export default function RegisterPremiumPage() {
  ...
  useEffect(() => {
    if (!loading && user && !success) {
      // If they are already logged in, just redirect them to premium
      router.push('/premium');
    }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Assign the 'premium' role upon registration
          ...
          <div className="relative border-2 border-red-600/60 p-1.5 rounded-none shadow-2xl overflow-hidden">
            ...
            <div className="relative bg-slate-900/80 backdrop-blur-xl rounded-none p-10 text-center">
              ...
              <p className="text-slate-300 font-medium">
                Your account has been successfully created and upgraded. Redirecting you to the Premium Dashboard...
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="relative border-2 border-red-600/60 p-1 rounded-none shadow-2xl overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center mb-4">
            <h1 className="text-[8px] font-bold text-white tracking-normal italic">
              Claim Your <span className="text-red-500">Premium</span> Access
            </h1>
            <p className="text-slate-400 mt-2 text-[6px] font-medium leading-tight">
              Thank you for your purchase! Create your account below to access the 10-Grid Advanced Predictor.
            </p>
          </div>
          ...
          <form onSubmit={handleRegister} className="flex flex-col space-y-2 relative z-10">
            <input 
              ...
              required
              className="w-full bg-slate-800 border border-slate-700 text-white px-2 py-1.5 rounded-none text-[7px] focus:outline-none focus:border-red-500 transition-colors"
            />
            <input 
              type="password" 
